...
class ManeuversProfile(Enum):
    LAUNCH = "launch"
    DOCKING = "docking"
    RENDEZVOUS = "redezvous"
    ORBIT = "orbit"
    DEORBIT = "deorbit"
    LANDING = "landing"
    DISCARD = "discard"

class Maneuvers(Protocol):
    async def execute():
        ...

class ManeuverLaunch(Maneuvers):
    async def execute():
        print("Maneuver Launch")

class ManeuverDocking(Maneuvers):
    async def execute():
        print("Maneuver Docking")

class ManeuverRendezvous(Maneuvers):
    async def execute():
        print("Maneuver Rendezvous")

class ManeuverOrbit(Maneuvers):
    async def execute():
        print("Maneuver Orbit")

class ManeuverDeorbit(Maneuvers):
    async def execute():
        print("Maneuver Deorbit")

class ManeuverLanding(Maneuvers):
    async def execute():
        print("Maneuver Landing")

class ManeuverDiscard(Maneuvers):
    async def execute():
        print("Maneuver Discard")

STAGES_PROFILE = {
    ManeuversProfile.LAUNCH : ManeuverLaunch,
    ManeuversProfile.ORBIT: ManeuverDocking,
    ManeuversProfile.RENDEZVOUS: ManeuverRendezvous,
    ManeuversProfile.DEORBIT : ManeuverDeorbit,
    ManeuversProfile.LANDING : ManeuverLanding,
    ManeuversProfile.DISCARD: ManeuverDiscard,
}
...
class ManeuverNode(object):
    ...
    async def execute(self, lead_time=15):
        conn = self.conn
        vessel = self.vessel
        node = self.node
